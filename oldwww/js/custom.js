...

{
    ...
    db.transaction(function (tx) {
        ...
        tx.executeSql(sortByDurationStatement, [], function (tx, result) {
            ...
            dataset = result.rows;
            var temp = '<form><input data-type="search" id="searchForCollapsibleSet"></form>';
            ...
            for (var i = 0, item = null; i < dataset.length; i++) {
                ...
                var linkeditdelete = '<div data-role="collapsible" data-collapsed-icon="arrow-d" data-expanded-icon="arrow-u" data-iconpos="right" data-theme="a" data-content-theme="b" data-mini="true" data-filtertext='+item['name']+'><h3 class="ui-li-heading">'+item['name']+'<span class="spellSortListsDuration">'+item['duration']+'</span></h3><ul data-role="listview"><li>School: '+item['school']+'</li><li>Level: '+item['level']+'</li><li>Casting Time: '+item['casting_time']+'</li><li>Range: '+item['range']+'</li><li>Duration: '+item['duration']+'</li></ul><p class="inset">'+item['description']+'</p><p class="inset">Components: '+item['components']+' </p></div>';
